...
export default function OffersPage() {
  ...
  const perPage = 5
  const filtered = useMemo(() => allJobs.filter((job) => (type === 'all' || (type === 'zlecenia' && job.kind === 'zlecenie') || (type === 'etat' && job.kind !== 'zlecenie')) && `${job.title} ${job.company} ${job.category} ${job.location} ${job.mode}`.toLowerCase().includes(query.toLowerCase()) && (mode === 'Wszystkie tryby' || job.mode === mode) && (category === 'Wszystkie kategorie' || job.category === category)), [query, mode, category, type])
  const pages = Math.max(1, Math.ceil(filtered.length / perPage))
  const visible = filtered.slice((page - 1) * perPage, page * perPage)

  useEffect(() => {
    const requestedType = new URLSearchParams(window.location.search).get('type')
    if (requestedType === 'zlecenia' || requestedType === 'etat') setType(requestedType)
  }, [])
